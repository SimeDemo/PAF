...
class Projectile {
    ...
    public:
        ...
        Projectile(float v, float theta, float rho, float C, float A, float m, float _x0, float _y0, bool cube, float radius, float side) {
    ...
    public:
        ...
        float dTheta(float vx, float vy) {

            return abs(atan(vy / vx));
        }

    public:

        float dA(float theta) {

            return area * (abs(sin(theta)) + cos(theta));
        }
    
    public:

        void __move_cube(float dt) {

            float dtheta = dTheta(vx.back(), vy.back());
            float dArea = dA(dtheta)     ;
            float v = sqrt(pow(vx.back(), 2) + pow(vy.back(), 2));
            float ax = acc_x_RK(v, 0) * cos(dtheta);
            if (atan(vy.back() / vx.back()) >= 0) {
                sig = 1;
            } else {
                sig = -1;
            }
            float ay = acc_y_RK(v, 0) * sin(dtheta) * sig;
            vx.push_back(vx.back() + ax * dt);
            x.push_back(x.back() + vx.back() + dt);
            vy.push_back(vy.back() + ay * dt);
            y.push_back(y.back() + vy.back() * dt);
...
void cube_vs_sphere(float dt) {

    vector<float> xsphere;
    vector<float> ysphere;
    vector<float> xcube;
    vector<float> ycube;

    Projectile p1 (10, 30, 1.227, 0.5, 0.1, 5, 0, 0, false, 0.1, 0.2);

    while (p1.y.back() >= 0) {
        p1.__move_cube(dt);
        xsphere.push_back(p1.x.back());
        ysphere.push_back(p1.y.back());
    };

    Projectile p2 (10, 30, 1.227, 0.5, 0.1, 5, 0, 0, true, 0.1, 0.2);

    while (p2.y.back() >= 0) {
        p1.__move_cube(dt);
        xcube.push_back(p1.x.back());
        ycube.push_back(p1.y.back());
    };

    ofstream myfile;
    myfile.open("xsphere.txt");
        for (int i = 0; i < xsphere.size(); i++) {
            myfile << xsphere.at(i) << " ";
        }
        myfile.close();

    myfile.open("ysphere.txt");
        for (int j = 0; j < ysphere.size(); j++) {
            myfile << ysphere.at(j) << " ";
        }
        myfile.close();
